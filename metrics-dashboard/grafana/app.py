...
class Container:
    ...
    def get_new_templating(self, templating, company):
        new_var = {
            "current": {"text": company, "value": company},
            "hide": 2,
            "label": "",
            "name": "enterprise",
            "options": [{"selected": True, "text": company, "value": company}],
            "query": company,
            "skipUrlSync": False,
            "type": "constant",
        }
    ...
    def generate_dashboards(self):
        ...
        for c in self.enterprise_names:
            ...
            file_name = os.path.join(dashboard_destination_path, company_name, f"{company_name}-dashboard.json")
    ...
    async def _make_rpc_request(self):
        rpc_request_msg = {"request_id": uuid(), "body": {"filter": ["Titan America|85940|"]}}
        logger.info("Requesting...")
        response = await self.event_bus.rpc_request("request.enterprises.names", rpc_request_msg, timeout=20)
