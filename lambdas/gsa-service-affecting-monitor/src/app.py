...
def handler(event, context):
    ...
    notes_repository = NotesRepository(config=config, trouble_repository=trouble_repository,
                                       utils_repository=utils_repository)

    # ACTIONS
    service_affecting_monitor = ServiceAffectingMonitor(
        logger=logger, config=config, velocloud_repository=velocloud_repository, notes_repository=notes_repository,
        trouble_repository=trouble_repository, utils_repository=utils_repository, email_repository=email_repository
    )

    asyncio.get_event_loop().run_until_complete(
        service_affecting_monitor.start_service_affecting_monitor()
    )
