...
class ServiceAffectingMonitor:
    def __init__(self, logger, config, velocloud_repository, notes_repository, trouble_repository, utils_repository,
                 email_repository):
        self._logger = logger
        self._config = config
    ...
    async def _report_trouble(self, link_data: dict, trouble: AffectingTroubles):
        ...
        self._logger.info(f'Sending an email to report {trouble.value} trouble '
                          f'on interface {interface} of edge {serial}')
