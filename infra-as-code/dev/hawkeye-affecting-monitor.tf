...
data "aws_ecr_repository" "automation-hawkeye-affecting-monitor" {
  name = "automation-hawkeye-affecting-monitor"
}

data "external" "hawkeye-affecting-monitor-build_number" {
  program = [
    "bash",
    "${path.module}/scripts/obtain_latest_image_for_repository.sh",
    data.aws_ecr_repository.automation-hawkeye-affecting-monitor.name
  ]
}

data "template_file" "automation-hawkeye-affecting-monitor" {
  template = file("${path.module}/task-definitions/hawkeye_affecting_monitor.json")

  vars = {
    image = local.automation-hawkeye-affecting-monitor-image
    ...
    PAPERTRAIL_PREFIX = local.automation-hawkeye-affecting-monitor-papertrail_prefix
    PAPERTRAIL_HOST = var.PAPERTRAIL_HOST
    PAPERTRAIL_PORT = var.PAPERTRAIL_PORT
  }
}

resource "aws_ecs_task_definition" "automation-hawkeye-affecting-monitor" {
  family = local.automation-hawkeye-affecting-monitor-ecs_task_definition-family
  container_definitions = data.template_file.automation-hawkeye-affecting-monitor.rendered
...
resource "aws_security_group" "automation-hawkeye-affecting-monitor_service" {
  vpc_id = data.aws_vpc.mettel-automation-vpc.id
  name = local.automation-hawkeye-affecting-monitor_service-security_group-name
  ...
  tags = {
    Name = local.automation-hawkeye-affecting-monitor-service-security_group-tag-Name
    Environment = var.ENVIRONMENT
  }
}

resource "aws_service_discovery_service" "hawkeye-affecting-monitor" {
  name = local.automation-hawkeye-affecting-monitor-service_discovery_service-name
...
resource "aws_ecs_service" "automation-hawkeye-affecting-monitor" {
  name = local.automation-hawkeye-affecting-monitor-resource-name
  task_definition = local.automation-hawkeye-affecting-monitor-task_definition
  desired_count = var.hawkeye_affecting_monitor_desired_tasks
  launch_type = "FARGATE"
  ...
  network_configuration {
    security_groups = [
      aws_security_group.automation-hawkeye-affecting-monitor_service.id]
    subnets = data.aws_subnet_ids.mettel-automation-private-subnets.ids
    assign_public_ip = false
  }

  service_registries {
    registry_arn = aws_service_discovery_service.hawkeye-affecting-monitor.arn
  }
