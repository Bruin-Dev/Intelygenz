...
data "external" "tnba-feedback-build_number" {
  program = [
    "bash",
    "${path.module}/scripts/obtain_latest_image_for_repository.sh",
    data.aws_ecr_repository.automation-tnba-feedback.name
  ]
}

data "template_file" "automation-tnba-feedback" {
  count = var.tnba_feedback_desired_tasks > 0 ? 1 : 0

  template = file("${path.module}/task-definitions/tnba_feedback.json")

  vars = {
    image = local.automation-tnba-feedback-image
    log_group = var.ENVIRONMENT
    log_prefix = local.automation-tnba-feedback-log_prefix
...
resource "aws_ecs_task_definition" "automation-tnba-feedback" {
  count = var.tnba_feedback_desired_tasks > 0 ? 1 : 0
  
  family = local.automation-tnba-feedback-ecs_task_definition-family
  container_definitions = data.template_file.automation-tnba-feedback[0].rendered
...
resource "aws_security_group" "automation-tnba-feedback_service" {
  ...
  tags = {
    Name = local.automation-tnba-feedback-service-security_group-tag-Name
    Environment = var.ENVIRONMENT
  }
}

resource "aws_service_discovery_service" "tnba-feedback" {
  count = var.tnba_feedback_desired_tasks > 0 ? 1 : 0

  name = local.automation-tnba-feedback-service_discovery_service-name
...
resource "aws_ecs_service" "automation-tnba-feedback" {
  count = var.tnba_feedback_desired_tasks > 0 ? 1 : 0

  name = local.automation-tnba-feedback-ecs_service-name
  task_definition = local.automation-tnba-feedback-ecs_service-task_definition
  desired_count = var.tnba_feedback_desired_tasks
  launch_type = "FARGATE"
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.tnba-feedback[0].arn
  }
