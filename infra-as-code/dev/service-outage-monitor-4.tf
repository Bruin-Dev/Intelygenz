data "template_file" "automation-service-outage-monitor-4" {
  ...
  vars = {
    container_name = local.automation-service-outage-monitor-4-container_name
...
resource "aws_ecs_task_definition" "automation-service-outage-monitor-4" {
  count = var.service_outage_monitor_4_desired_tasks > 0 ? 1 : 0
  family = local.automation-service-outage-monitor-4-ecs_task_definition-family
  container_definitions = data.template_file.automation-service-outage-monitor-4[0].rendered
...
resource "aws_security_group" "automation-service-outage-monitor-4_service" {
  ...
  tags = {
    Name = local.automation-service-outage-monitor-4-service-security_group-tag-Name
    Environment = var.ENVIRONMENT
  }
}

resource "aws_service_discovery_service" "service-outage-monitor-4" {
  count = var.service_outage_monitor_4_desired_tasks > 0 ? 1 : 0
  name = local.automation-service-outage-monitor-4-service_discovery_service-name
...
resource "aws_ecs_service" "automation-service-outage-monitor-4" {
  name = local.automation-service-outage-monitor-4-ecs_service-name
  task_definition = local.automation-service-outage-monitor-4-ecs_service-task_definition
  desired_count = var.service_outage_monitor_4_desired_tasks
  launch_type = "FARGATE"
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.service-outage-monitor-4[0].arn
  }
