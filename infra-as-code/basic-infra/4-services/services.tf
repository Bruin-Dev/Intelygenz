data "aws_kms_key" "ssm-kms-key" {
  key_id = "alias/kms-key-${var.CURRENT_ENVIRONMEN}-parameters"
}
...
module "service_affecting_monitor" {
  ...
  service_affecting_monitor_s3_bucket_name = aws_s3_bucket.service_affecting_monitor.name
  ...
  global_ssm_kms_name                      = data.aws_kms_key.ssm-kms-key.name
}
