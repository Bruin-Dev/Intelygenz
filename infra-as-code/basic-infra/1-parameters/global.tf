...
resource "aws_ssm_parameter" "parameter-ticket-task-worker-queue-id" {
  name        = "/automation-engine/${local.env}/ticket-task-worker-queue-id"
  description = "Ticket queue id for assigning to dedicated workers queue"
  type        = "SecureString"
  value       = "-" # to edit go to parameter store dashboard.
  key_id      = aws_kms_alias.kms_key.name
  ...
  tags = merge(var.common_info, {
    Name = "TICKET_TASK_WORKER_QUEUE_ID"
    note = "can be updated from the parameter store dashboard"
  })
}

resource "aws_ssm_parameter" "parameter-ticket-task-ipa-queue-id" {
  name        = "/automation-engine/${local.env}/ticket-task-ipa-queue-id"
  description = "Ticket queue id for assigning to dedicated ipa queue"
  type        = "SecureString"
  value       = "-" # to edit go to parameter store dashboard.
  key_id      = aws_kms_alias.kms_key.name
