...
resource "aws_ssm_parameter" "parameter-intermapper-outage-monitor-observed-inbox-email-addresses" {
  name        = "/automation-engine/${local.env}/intermapper-outage-monitor/observed-inbox-email-addresses"
  description = "E-mail accounts that receives InterMapper events for later analysis"
  type        = "SecureString"
  value       = "-" # to edit go to parameter store dashboard.
  key_id      = aws_kms_alias.kms_key.name
  ...
  tags = merge(var.common_info, {
    Name = "OBSERVED_INBOX_EMAIL_ADDRESSES"
    note = "can be updated from the parameter store dashboard"
  })
}
