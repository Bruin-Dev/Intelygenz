resource "aws_ssm_parameter" "parameter-email-bridge-email-account-for-message-delivery-password" {
  ...
  name        = "/automation-engine/common/email-bridge/email-account-for-message-delivery-password"
...
resource "aws_ssm_parameter" "parameter-email-bridge-email-account-for-message-delivery-username" {
  count       = var.CURRENT_ENVIRONMENT == "dev" ? 1 : 0   # -> use this to deploy a "common" parameter only in one environment, if not when merging to master will fail for duplicity
  name        = "/automation-engine/common/email-bridge/email-account-for-message-delivery-username"
...
resource "aws_ssm_parameter" "parameter-email-bridge-monitorable-email-accounts" {
  name        = "/automation-engine/${local.env}/email-bridge/monitorable-email-accounts"
