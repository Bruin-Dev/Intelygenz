...
resource "null_resource" "create-aws-secret-cert-manager" {

  provisioner "local-exec" {
    command = "kubectl --namespace=cert-manager create secret generic aws-secret --from-literal=AWS_SECRET_ACCESS_KEY=${var.AWS_SECRET_ACCESS_KEY} -o yaml --dry-run | kubectl apply -f -"
  }
