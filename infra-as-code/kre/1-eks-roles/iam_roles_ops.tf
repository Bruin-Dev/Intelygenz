...
resource "aws_iam_role" "ops_eks" {
  count                 = length(var.eks_ops_users)
  name                  = "${substr(var.CURRENT_ENVIRONMENT, 0, 3)}-ops-${var.common_info.project}-${var.eks_ops_users[count.index]}"
...
resource "aws_iam_policy" "assume-ops-role" {
  count  = length(var.eks_ops_users)
  name   = "policy-${substr(var.CURRENT_ENVIRONMENT, 0, 3)}-ops-${var.common_info.project}-${var.eks_ops_users[count.index]}"
