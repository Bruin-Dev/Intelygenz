...
class TestEventBus:
    ...
    async def connect_OK_test(self):
        e = EventBus()
        subscriber = NatsStreamingClient(config, "Some-subs-ID")
        publisher = NatsStreamingClient(config, "Some-pub-ID")
    ...
    def add_consumer_KO_repeated_test(self):
        e = EventBus()
        first_subscriber = NatsStreamingClient(config, "Some-subs-ID")
        second_subscriber = NatsStreamingClient(config, "Some-subs-ID-2")
    ...
    async def subscribe_consumer_OK_test(self):
        ...
        action_mock = ActionWrapper(None, "")
