...
class RedisStorageManager(MessageStorageManager):
    def store_message(self, msg, encode_result=False):
        ...
        self._logger.info(f'Storing message (truncated payload: "{msg[:200]}...") within Redis...')

        uuid_ = uuid()
        self._storage_client.set(name=uuid_, value=msg, ex=300)

        self._logger.info(f'Message (truncated) "{msg[:200]}..." successfully stored within Redis')
    ...
    def recover_message(self, msg, encode_result=False):
        ...
        try:
            ...
            self._logger.info(f'Message (truncated) "{stored_msg[:200]}..." successfully obtained from Redis')
