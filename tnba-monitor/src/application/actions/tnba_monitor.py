                ...
                if self._is_ticket_old_enough(ticket_creation_date):
                    ...
                    if self._ticket_repository.is_detail_in_outage_ticket(detail_object):
                        ...
                        if newest_tnba_note and self._config.ENVIRONMENT == 'production':
                            ...
                            self._logger.info(msg)
                            response = await self._bruin_repository.change_detail_work_queue(
                                ticket_id,
                            ...
                            # Retries?
                            self._logger.info(
                                f"The status of the response to change queue for serial: {serial_number} "
                                f"and ticket: {ticket_id} is: {response['status']}")
