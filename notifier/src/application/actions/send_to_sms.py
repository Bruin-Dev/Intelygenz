...
class SendToSms:
    ...
    async def send_to_sms(self, msg: dict):
        ...
        if msg_body is not None \
            ...
            response = self._telestax_repository.send_to_sms(msg_body["sms_data"], msg_body["sms_to"])
            status = 200
        notification_response = {"request_id": msg['request_id'], "status": status, "body": response}
