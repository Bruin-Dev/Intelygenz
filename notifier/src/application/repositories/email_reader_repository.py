class EmailReaderRepository:
    ...
    def get_unread_emails(self, email_account, email_filter):
        status = 500
        if email_account in self._config.EMAIL_ACCOUNTS.keys():
            email_password = self._config.EMAIL_ACCOUNTS[email_account]
            unread_emails = self._email_reader_client.get_unread_messages(email_account, email_password, email_filter)
    ...
    def mark_as_read(self, msg_uid, email_account):
        status = 500
        body = f'Failed to mark message {msg_uid} as read'
        if email_account in self._config.EMAIL_ACCOUNTS.keys():
            email_password = self._config.EMAIL_ACCOUNTS[email_account]
            marked_as_read = self._email_reader_client.mark_as_read(msg_uid, email_account, email_password)
