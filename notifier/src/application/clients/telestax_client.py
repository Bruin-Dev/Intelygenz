...
class TeleStaxClient:
    ...
    def send_to_sms(self, msg, sms_to):
        @retry(wait=wait_fixed(self._config.TELESTAX_CONFIG['wait_fixed']),
               stop=stop_after_attempt(self._config.TELESTAX_CONFIG['stop_after_attempt']),
               reraise=True)
        def send_to_sms():
            sms_response = dict()
            ...
            try:
                ...
                # return sms_response
                raise Exception(f'Error: SMS not sent, exception: {ex}')
        try:
            return send_to_sms()
