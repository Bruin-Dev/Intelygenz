...
class EmailReaderClient:
    ...
    def get_unread_messages(self, email_account, email_password, email_filter):
        self._login(email_account, email_password)
        unread_messages = []
        messages = []
        for sender_email in email_filter:
            messages = messages + self._search_messages(email_account, email_password, sender_email)

        for num in messages:
            data = self._extract_data_from_message(num)
            if data is not None:
    ...
    def _mark_as_unread(self, msg_uid, email_account, email_password):
    ...
    def mark_as_read(self, msg_uid, email_account, email_password):
