...
if (config.activatedMocks) {
  const mock = new MockAdapter(axiosInstance, { delayResponse: 1000 });

  if (config.activatedMocks.includes(config.allMocks.LOGIN)) {
    mock.onPost(API_URLS.LOGIN).reply(200, {
      token: 'XXXX-fake-token'
    });
  }

  if (config.activatedMocks.includes(config.allMocks.GET_ALL_DISPATCHES)) {
  ...
  if (config.activatedMocks.includes(config.allMocks.CREATE_DISPATCH)) {
    mock.onPost(API_URLS.DISPATCH).reply(204, { id: 123 });
  }

  mock.onPost(API_URLS.UPLOAD_FILES).reply(204);

  if (config.activatedMocks.includes(config.allMocks.GET_DISPATCH)) {
