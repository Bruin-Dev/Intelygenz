...
class Container:
    ...
    def __init__(self):
        ...
        self._logger.info(f'TNBA feedback starting in {config.TNBA_FEEDBACK_CONFIG["environment"]}...')
        ...
        # SCHEDULER
        self._scheduler = AsyncIOScheduler(timezone=config.TNBA_FEEDBACK_CONFIG['timezone'])
        ...
        self._customer_cache = CustomerCacheRepository(event_bus=self._event_bus, logger=self._logger, config=config,
                                                       notifications_repository=self._notifications_repository)
