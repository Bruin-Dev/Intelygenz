...
class ServiceAffectingMonitor:
    ...
    def __init__(self, event_bus: EventBus, logger, scheduler, config, notifier_action, template_renderer):
        self._event_bus = event_bus
        self._logger = logger
        self._scheduler = scheduler
        self._config = config
        self._monitoring_minutes = config.MONITOR_CONFIG["monitoring_minutes"]
        self._notifier_action = notifier_action
        ...
        if edge_status is None or \
            ...
            slack_message = {'request_id': uuid(),
                             'message': f'Error while monitoring edge for service affecting trouble, seems like data '
                                        ...
                                        f'The environment is {self._config.ENV_CONFIG["environment"]}'}
    ...
    async def _notify_trouble(self, device, edge_status, link, input, output, trouble, threshold):
        ...
        await self._notifier_action.run_action(device, edge_status, trouble, ticket_dict)
