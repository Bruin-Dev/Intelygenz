...
class TestGetCustomers:
    ...
    async def get_customers_200_test(self, get_customer, cache_probes_now, default_call_with_params):
        ...
        get_customer._event_bus.publish_message.assert_awaited_once_with(
            default_call_with_params['response_topic'],
            dict(request_id='1234',
                 body=cache_probes_now,
                 status=200))

    @pytest.mark.asyncio
    async def get_customers_not_found_probes_for_filter_test(self, get_customer, default_call_with_params,
                                                             response_not_found_message, cache_probes_now):
        get_customer._storage_repository.get_hawkeye_cache = Mock(return_value=cache_probes_now)
    ...
    @pytest.mark.asyncio
    async def get_customers_no_cache_test(self, get_customer, default_call_with_params,
