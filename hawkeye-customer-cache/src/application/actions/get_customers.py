...
class GetCustomers:
    ...
    async def get_customers(self, msg: dict):
        ...
        if len(cache) == 0:
            ...
            return
        filter_cache = [device for device in cache if
                        parse(last_contact_filter).astimezone(utc) < parse(device['last_contact']).astimezone(
                            utc)] if last_contact_filter is not None else cache
