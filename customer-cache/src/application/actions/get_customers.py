...
class GetCustomers:
    ...
    async def get_customers(self, msg: dict):
        ...
        if "body" not in msg.keys():
            self._logger.error(f'Cannot get customer cache using {json.dumps(msg)}. '
                               f'JSON malformed')
            response["status"] = 400
            response["body"] = 'You must specify ' \
                               '{.."body":{"filter":[...]}} in the request'
        ...
        if len(caches) == 0:
            ...
            return
        filter_cache = [edge for edge in caches if
                        parse(last_contact_filter).astimezone(utc) < parse(edge['last_contact']).astimezone(
                            utc)] if last_contact_filter is not None else caches
