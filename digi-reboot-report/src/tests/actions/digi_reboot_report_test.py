...
class TestDiGiRebootReport:
    ...
    def instance_test(self):
        ...
        digi_reboot_report = DiGiRebootReport(event_bus, scheduler, logger, config, bruin_repository, digi_repository,
    ...
    async def start_digi_reboot_report_job_with_exec_on_start_test(self):
        ...
        scheduler.add_job.assert_called_once_with(
            digi_reboot_report._digi_reboot_report_process, 'interval',
    ...
    async def digi_reboot_report_process_test(self):
        digi_recovery_logs_response = {
            'body': {'Logs': [{'TicketID': '123'}, {'TicketID': '321'}]},
            'status': 200
        }
        ...
        ticket_task_history_map = {
            '123': {"ClientName": "Le Duff Management ",
    ...
    async def _get_ticket_task_histories_test(self):
        ...
        parsed_ticket_history = [{'reboot_attempted': True, 'reboot_time': datetime_now - timedelta(days=2)},
        ...
        bruin_repository = Mock()
        bruin_repository.get_ticket_task_history = CoroutineMock(side_effect=[task_history_response_1,
                                                                              task_history_response_2,
                                                                              task_history_response_3,
                                                                              task_history_response_4])
        digi_repository = Mock()
        ...
        bruin_repository.get_ticket_task_history.assert_has_awaits(
           [call(ticket_id_list[0]),
            call(ticket_id_list[1]),
            call(ticket_id_list[2]),
            call(ticket_id_list[3])]
        )
    ...
    async def generate_and_email_csv_file_test(self):
        ...
        expected_breakdown = [day, 6, 2, 2, 2, 6, 5, 5, 270.0, 0, 0, 1, 1, 2, 1, 2, 2, 0, 0, 5, 1, 1, 1, 1, 2, 2, 1,
                              5, 2, 2, 1]
        event_bus = Mock()
        scheduler = Mock()
        logger = Mock()
