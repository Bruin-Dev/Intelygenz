...
from application import EVENT_INTERFACE_NAME_REGEX, Outages
        ...
        *,
        ...
        for link in edge_links:
            ...
            last_online_event_for_current_link = self._utils_repository.get_first_element_matching(
                ...
                condition=lambda event: event["event"] == "LINK_ALIVE"
                and self.__event_message_contains_interface_name(event["message"], interface_name),
            )

            last_offline_event_for_current_link = self._utils_repository.get_first_element_matching(
                iterable=edge_events,
                condition=lambda event: event["event"] == "LINK_DEAD"
                and self.__event_message_contains_interface_name(event["message"], interface_name),
            )
