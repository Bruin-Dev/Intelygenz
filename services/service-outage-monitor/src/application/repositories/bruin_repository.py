...
class BruinRepository:
    ...
    async def get_ticket_overview(self, ticket_id: int):
        ...
        try:
            logger.info(f"Getting overview for ticket id {ticket_id} ...")
            response = await self._nats_client.request(
            ...
            if response_status in range(200, 300):
                logger.info(f"Got overview of ticket {ticket_id}!")
            else:
                err_msg = (
    ...
    async def change_ticket_severity(self, ticket_id: int, severity_level: int, reason_for_change: str):
        ...
        try:
            ...
            response = await self._nats_client.request(
                "bruin.change.ticket.severity", to_json_bytes(request), timeout=45
            )
            response = json.loads(response.data)
        except Exception as e:
            err_msg = (
    ...
    async def change_ticket_severity_for_disconnected_links(self, ticket_id: int, links: List[str]):
        ...
        reason_for_change_lines += [f"Interface {link} Status: Disconnected" for link in links]
