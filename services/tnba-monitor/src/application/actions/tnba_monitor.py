...
class TNBAMonitor:
    ...
    async def _get_all_open_tickets_with_details_for_monitored_companies(self):
        ...
        bruin_clients_ids: Set[int] = set(
            elem["bruin_client_info"]["client_id"] for elem in list(self._customer_cache_by_serial.values())
        )
        tasks = [
            self._get_open_tickets_with_details_by_client_id(client_id, open_tickets) for client_id in bruin_clients_ids
        ]
        await asyncio.gather(*tasks)
        return open_tickets
        ...
        async with self._semaphore:
            ...
            if self._prediction_repository.is_request_or_repair_completed_prediction(best_prediction):
                ...
                autoresolve_status = await self._autoresolve_ticket_detail(
                    detail_object=detail_object, best_prediction=best_prediction
                )
    ...
    async def _autoresolve_ticket_detail(
        ...
        await self._bruin_repository.unpause_ticket_detail(
            ticket_id, service_number=serial_number, detail_id=ticket_detail_id
        )
    ...
    def _is_there_an_outage(self, edge_status: dict) -> bool:
        ...
        is_there_any_faulty_link = any(
            self._is_faulty_link(link_status["linkState"]) for link_status in edge_status["links"]
        )
