...
class OutageMonitor:
    ...
    async def _outage_monitoring_process(self):
        ...
        outage_devices = [
            device for device in probes_with_cache_info if self._is_there_an_outage(device["device_info"])
        ]
    ...
    def _schedule_recheck_job_for_devices(self, devices: list):
        ...
        self._scheduler.add_job(
            self._recheck_devices_for_ticket_creation,
            "date",
            args=[devices],
            run_date=run_date,
            replace_existing=False,
    ...
    async def _reopen_outage_ticket(self, ticket_id, device):
        ...
        ticket_reopening_response = await self._bruin_repository.open_ticket(
            ticket_id, ticket_detail_for_reopen["detailID"]
        )
