...
class DiGiRebootReport:
    ...
    def _merge_recovery_logs(self, ticket_map):
        ...
        for process in self._digi_recovery_logs:
            ...
            ticket_map[ticket_id]['process_start'] = datetime.strptime(
                process['TimestampSTART'], '%Y-%m-%dT%H:%M:%SZ') if process.get('TimestampSTART') is not None else None
            ticket_map[ticket_id]['process_end'] = datetime.strptime(
                process['TimestampEND'], '%Y-%m-%dT%H:%M:%SZ') if process.get('TimestampEND') is not None else None
            ...
            if ticket_map[ticket_id]['process_end'] is not None:
                ...
                ticket_map[ticket_id]['process_length'] = (ticket_map[ticket_id]['process_end'] - ticket_map[
                    ticket_id]['process_start']).total_seconds() / 60 if ticket_map[ticket_id][
                    'process_start'] else None
                if autoresolve_time:
    ...
    def _merge_process_length_avg(self, breakdown, process_length_by_day):
        ...
        for day in process_length_by_day:
            breakdown[day][8] = round(process_length_by_day[day]['total'] / (process_length_by_day[day]['count'] * 1.0),
                                      2)
    ...
    async def _generate_and_email_csv_file(self, ticket_map):
        ...
        fields = ['Time (EST)',									# 0
                  ...
                  'Unclassified Outage Tasks Forwarded']		# 30
