...
class TestBruinRepository:
    ...
    async def get_task_id_from_dri_ok_test(self):
        ...
        task_id_from_dri_response = {
            "body": {"status": "SUCCESS", "message": "All Good!", "data": {"Id": task_id_return,
                                                                           "ErrorCode": 100,
                                                                           "Message": ""}},
            "status": 200
    ...
    async def get_task_id_from_dri_ko_200_failed_status_test(self):
        ...
        dri_task_id_body = {'status': 'FAILED', 'message': 'All Good!', 'data': {
                            'Id': 1725608, 'ErrorCode': 100, 'Message': ''}}
        task_id_from_dri_response = {
            "body": dri_task_id_body,
            "status": 200

        }

        get_task_id_from_dri_return_dict = {
            'body': dri_task_id_body,
            'status': 400
        }
        logger = Mock()
    ...
    async def get_task_results_ko_rejected_test(self):
        ...
        assert task_results_response == {"body": f"DRI task was rejected for serial {serial}",
                                         "status": 403}
    ...
    async def get_task_results_ko_200_failed_test(self):
        ...
        get_tasks_result_response = {
            "body": f"Failed to retrieve data from DRI for serial {serial}",
            "status": 400
    ...
    async def get_pending_task_ids_ko_200_failed_test(self):
        ...
        pending_task_from_dri_body = {'status': 'FAILED', 'message': 'All Good!', 'data': {
                'Transactions': [],
                'ErrorCode': 100,
                'Message': ''}}

        pending_task_from_dri_response = {
            'body': pending_task_from_dri_body,
            'status': 200
        }
        pending_task_ids_response = {
            "body": pending_task_from_dri_body,
