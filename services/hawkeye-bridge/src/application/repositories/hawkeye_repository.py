...
class HawkeyeRepository:
    ...
    async def get_test_results(self, probes_uids, interval):
        ...
        for probe_uid in probes_uids:
            all_test_results_response = await self.__make_paginated_request(
                self._hawkeye_client.get_tests_results,
                {"startDate": interval["start"], "endDate": interval["end"], "limit": 100, "probeFrom": probe_uid},
            )
