...
class TestHawkeyeRepository:
    def instance_test(self, hawkeye_client):
        ...
        hawkeye_repository = HawkeyeRepository(hawkeye_client=hawkeye_client, config=config)
    ...
    async def get_probes_ok_test(self, hawkeye_repository, default_call_with_params, response_get_probes_down_ok):
        hawkeye_repository._hawkeye_client.get_probes = AsyncMock(side_effect=[response_get_probes_down_ok])
    ...
    @pytest.mark.asyncio
    async def get_probes_big_ok_test(
        self,
        hawkeye_repository,
        default_call_with_params,
        first_response_get_probes_down_big_ok,
        second_response_get_probes_down_big_ok,
        third_response_get_probes_down_big_ok,
    ):
        hawkeye_repository._hawkeye_client.get_probes = AsyncMock(
            side_effect=[
                first_response_get_probes_down_big_ok,
                second_response_get_probes_down_big_ok,
                third_response_get_probes_down_big_ok,
    ...
    async def get_probes_ko_test(self, hawkeye_repository, default_call_with_params, response_bad_status):
        hawkeye_repository._hawkeye_client.get_probes = AsyncMock(return_value=response_bad_status)
        hawkeye_repository = init_self_and_name_get_probes(hawkeye_repository)
        result = await hawkeye_repository.get_probes(default_call_with_params)
        assert result["body"] == []
        assert result["status"] == 200
    ...
    @pytest.mark.asyncio
    async def get_probes_check_retries_test(
        self, hawkeye_repository, default_call_with_params, response_get_probes_down_not_end_ok, response_bad_status
    ):
        hawkeye_repository._hawkeye_client.get_probes = AsyncMock(
            side_effect=[
    ...
    @pytest.mark.asyncio
    async def get_probes_check_retries_2_test(
        self,
        hawkeye_repository,
        default_call_with_params,
    ...
    @pytest.mark.asyncio
    async def get_test_results_test(
        self,
        hawkeye_repository,
        get_response_test_result,
        get_response_test_result_details,
        get_response_test_result_details_empty,
    ):
        probes_id = ["b8:27:eb:76:a8:de"]
        hawkeye_repository._HawkeyeRepository__make_paginated_request = AsyncMock(return_value=get_response_test_result)
        hawkeye_repository._hawkeye_client.get_test_result_details = AsyncMock(
            side_effect=[get_response_test_result_details, get_response_test_result_details_empty]
        )
        result = await hawkeye_repository.get_test_results(
            probes_id, {"start": "fake_start_date", "end": "fake_end_date"}
        )
        assert result["body"][probes_id[0]] == [
            {
                "summary": get_response_test_result["body"][0],
