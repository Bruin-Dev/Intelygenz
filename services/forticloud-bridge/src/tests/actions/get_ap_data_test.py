...
def action(forticloud_repository):
    return GetApData(forticloud_repository=forticloud_repository)
...
class TestGetApData:
    ...
    @pytest.mark.asyncio
    async def get_ap_data_ok_test(self, action):
        PAYLOAD_MODEL = {
        ...
        action._forticloud_repository.get_ap_data = AsyncMock(return_value=expected_response)

        await action(msg=msg_mock)

        action._forticloud_repository.get_ap_data.assert_awaited_once_with(payload=PAYLOAD_MODEL)
