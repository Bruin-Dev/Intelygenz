...
class GetAttributeSerial:
    ...
    async def get_attributes_serial(self, msg: dict):
        ...
        if "body" not in msg.keys():
            self._logger.error(f"Cannot get attribute's serial number using {json.dumps(msg)}. "
                               f'JSON malformed')
            response["status"] = 400
        ...
        if not all(key in filters.keys() for key in ("client_id", "status", "service_number")):
            self._logger.info(f"Cannot get attribute's serial number using {json.dumps(filters)}. "
        ...
        self._logger.info(
            f"'Getting attribute's serial number with filters: {json.dumps(filters)}'"
        )
