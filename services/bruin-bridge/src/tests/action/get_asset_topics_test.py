...
class TestGetAssetTopics:
    ...
    async def get_asset_topics_ok_test(self, get_asset_topics, mocked_response, event_bus):
        ...
        event_bus.publish_message.assert_awaited_once_with(
            "any_response_topic",
            {"request_id": "any_request_id", "status": mocked_response.status, "body": mocked_response.body},
        )
    ...
    async def get_asset_topics_without_body_test(self, get_asset_topics, event_bus):
        ...
        # Then
        event_bus.publish_message.assert_awaited_once_with(
            ANY, {"request_id": ANY, "status": HTTPStatus.BAD_REQUEST, "body": NO_BODY_MSG}
        )
    ...
    async def get_asset_topics_without_client_id_test(self, get_asset_topics, event_bus):
        ...
        # Then
        event_bus.publish_message.assert_awaited_once_with(
            ANY, {"request_id": ANY, "status": HTTPStatus.BAD_REQUEST, "body": MISSING_PARAMS_MSG}
        )
    ...
    async def get_asset_topics_with_string_client_id_test(self, get_asset_topics, event_bus):
        ...
        # Then
        event_bus.publish_message.assert_awaited_once_with(
            ANY, {"request_id": ANY, "status": HTTPStatus.BAD_REQUEST, "body": WRONG_CLIENT_ID_MSG}
        )
    ...
    async def get_asset_topics_with_empty_service_number_test(self, get_asset_topics, event_bus):
        ...
        # Then
        event_bus.publish_message.assert_awaited_once_with(
            ANY, {"request_id": ANY, "status": HTTPStatus.BAD_REQUEST, "body": EMPTY_SERVICE_NUMBER_MSG}
        )
