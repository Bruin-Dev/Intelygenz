...
class GetPrediction:
    ...
    async def get_prediction(self, msg: dict):
        ...
        if not email_data:
            self._logger.error(f'Cannot get prediction using {json.dumps(msg_body)}. Need parameters "email"')
            response["body"] = err_body
            response["status"] = 400
            await self._event_bus.publish_message(response_topic, response)
            return
        ...
        if not email_id:
            self._logger.error(f'Cannot get prediction using {json.dumps(msg_body)}. Need parameters "email_id"')
            response["body"] = err_body
            response["status"] = 400
            await self._event_bus.publish_message(response_topic, response)
            return
