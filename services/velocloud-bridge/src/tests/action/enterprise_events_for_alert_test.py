...
def action(velocloud_repository):
    return EventEnterpriseForAlert(velocloud_repository=velocloud_repository)
...
def missing_filters_response():
    return {
        "body": 'Must include "enterprise_id", "host", "start_date", "end_date" in request in body',
        "status": 400,
    }
...
async def ok_all_optional_filters_filled_in_test(action, any_msg, any_payload, any_response):
    # Given
    action._velocloud_repository.get_all_enterprise_events = AsyncMock(return_value=any_response)
    ...
    # Then
    action._velocloud_repository.get_all_enterprise_events.assert_awaited_once_with(
        host=payload_json["body"]["host"],
        enterprise=payload_json["body"]["enterprise_id"],
...
async def ko_body_missing_in_request_test(action, any_msg, no_body_response):
    # Given
    action._velocloud_repository.get_all_enterprise_events = AsyncMock()
    ...
    # Then
    action._velocloud_repository.get_all_enterprise_events.assert_not_awaited()
