...
class EventEnterpriseForAlert:
    ...
    async def report_enterprise_event(self, msg: dict):
        enterprise_event_response = {"request_id": msg['request_id'], "body": None,
                                     "status": None}
        ...
        if all(key in msg['body'].keys() for key in ("enterprise_id", "host", "start_date", "end_date")):
            ...
            events_by_enterprise = await self._velocloud_repository.get_all_enterprise_events(enterprise_id, host,
                                                                                              start, end,
                                                                                              limit, filter)
