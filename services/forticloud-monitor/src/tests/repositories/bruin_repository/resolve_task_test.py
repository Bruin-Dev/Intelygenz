...
async def tasks_are_properly_resolved_test(any_bruin_repository, any_task, any_response):
    any_task.id = "any_task_id"
    send = AsyncMock(return_value=any_response)
    bruin_repository = any_bruin_repository(send=send)

    # when
    await bruin_repository.resolve_task(ticket_id="any_ticket_id", task=any_task)

    # then
    send.assert_awaited_once_with(
        BruinRequest(
            method="POST",
            path="/api/Ticket/any_ticket_id/details/any_task_id/status",
            json={"Status": "R"},
        )
    )


async def client_errors_are_properly_propagated_test(any_bruin_repository, any_exception, any_task):
    # given
...
async def unexpected_response_status_raise_an_exception_test(any_bruin_repository, any_response, any_task):
    # given
