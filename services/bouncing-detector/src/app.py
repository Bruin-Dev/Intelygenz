...
class Container:
    ...
    def __init__(self):
        ...
        self._logger.info(f'Bouncing Detector starting in {config.BOUNCING_DETECTOR_CONFIG["environment"]}...')
        ...
        # SCHEDULER
        self._scheduler = AsyncIOScheduler(timezone=config.BOUNCING_DETECTOR_CONFIG['timezone'])
        ...
        # ACTIONS
        self._bouncing_detector = BouncingDetector(self._event_bus, self._logger, self._scheduler,
                                                   config, self._bruin_repository, self._velocloud_repository,
                                                   self._customer_cache_repository, self._notifications_repository)

    async def _start(self):
        await self._event_bus.connect()

        await self._bouncing_detector.start_bouncing_detector_job(exec_on_start=True)
