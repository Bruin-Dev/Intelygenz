...
class T7KRERepository:
    ...
    def get_prediction(self, ticket_id: int, ticket_rows: List[dict], assets_to_predict: List[str]) -> dict:
        camel_ticket_rows = list(map(self.__row_dict_to_camel, ticket_rows))
        ...
        if prediction["status"] in range(200, 300):
            ...
            body_prediction_response = list(
                map(
                    lambda p: {"assetId": p["asset"], "predictions": p["task_results"]}
                    if "task_results" in p
                    else {"assetId": p["asset"], "error": p["error"]},
                    predictions,
                )
            )
    ...
    def post_automation_metrics(self, params: dict) -> dict:
        ...
        camel_ticket_rows = list(map(self.__row_dict_to_camel, params["ticket_rows"]))
