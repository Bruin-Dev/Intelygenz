...
class _TimeZoneNaive(Formatter):
    converter = time.gmtime


class Standard(_TimeZoneNaive):
    def __init__(self, *, environment_name: str):
        fmt = (
            f"%(asctime)s: {environment_name}: {socket.gethostname()}: "
            f"%(module)s::%(lineno)d %(levelname)s: %(message)s"
        )
        super().__init__(fmt=fmt)


class Papertrail(_TimeZoneNaive):
    def __init__(self, *, environment_name: str, papertrail_prefix: str):
        fmt = (
            f"%(asctime)s: {environment_name}: {socket.gethostname()}: {papertrail_prefix}: "
...
class Json(_TimeZoneNaive):
    """
    Simple logging formatter using JSON serialization.
    Adapted from https://github.com/velebit-ai/useful-logs/blob/master/useful/logs/_json_logging.py
    """
