...
class VelocloudRepository:
    ...
    def get_edge_information(self, host, enterpriseid, edgeid):
        edge_information = self._velocloud_client._get_edge_information(self._clients, self._logger,
                                                                        host, enterpriseid, edgeid)
        return edge_information

    def get_link_information(self, host, enterpriseid, edgeid):
        link_information = self._velocloud_client._get_link_information(self._clients, self._logger,
                                                                        host, enterpriseid, edgeid)
        return link_information

    def get_enterprise_information(self, host, enterpriseid):
        enterprise_information = self._velocloud_client._get_enterprise_information(self._clients, self._logger,
                                                                                    host, enterpriseid)
