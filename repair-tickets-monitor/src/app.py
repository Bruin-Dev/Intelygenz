...
from application.repositories.repair_tickets_kre_repository import (
    RepairTicketsKRERepository,
)

from application.actions.repair_tickets_monitor import RepairTicketsMonitor
from application.actions.repair_tickets_feedback_monitor import (
    RepairTicketsFeedbackMonitor,
)
...
class Container:
    def __init__(self):
        ...
        self._redis_client = redis.Redis(
            host=config.REDIS["host"], port=6379, decode_responses=True
        )
        self._redis_client.ping()
        self._message_storage_manager = RedisStorageManager(
            self._logger, self._redis_client
        )

        # Redis for data storage
        self._redis_cache_client = redis.Redis(
            host=config.REDIS_CACHE["host"], port=6379, decode_responses=True
        )
        self._redis_cache_client.ping()

        self._scheduler = AsyncIOScheduler(
            timezone=timezone(config.MONITOR_CONFIG["timezone"])
        )
        ...
        self._storage_repository = StorageRepository(
            config, self._logger, self._redis_cache_client
        )
        ...
        self._repair_tickets_repository = RepairTicketsRepository(
            self._logger,
            config,
            self._notifications_repository,
            self._storage_repository,
        )
        self._repair_tickets_kre_repository = RepairTicketsKRERepository(
            self._logger, config, self._event_bus, self._notifications_repository
        )
    ...
    async def _start(self):
        ...
        await self._repair_tickets_monitor.start_repair_tickets_monitor(
            exec_on_start=True
        )
