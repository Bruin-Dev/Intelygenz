...
class TestLitClient:
    ...
    def get_all_dispatches_test(self):
        ...
        with patch.object(lit_client._salesforce_sdk, 'apexecute',
                          return_value=expected_dispatch_response) as mock_apexecute:
            response = lit_client.get_all_dispatches()
            mock_apexecute.assert_called_once()
            assert response["body"] == expected_dispatch_response
            assert response["status"] == HTTPStatus.OK

    def get_all_dispatches_with_error_400_test(self):
        logger = Mock()
        config = testconfig
        logger.error = Mock()
    ...
    def get_all_dispatches_with_error_500_test(self):
        logger = Mock()
        config = testconfig
        logger.error = Mock()
