...
from application.repositories import edge_redis_repository as edge_redis_repository_module
...
class TestEdgeRedisRepository:
    ...
    def instance_test(self):
        ...
        edge_redis_repository = EdgeRedisRepository(redis_client, keys_prefix, logger)

        assert edge_redis_repository._logger is logger
        assert edge_redis_repository._redis_client is redis_client
        assert edge_redis_repository._keys_prefix is keys_prefix
    ...
    def remove_all_stored_elements_test(self):
        ...
        edge_redis_repository = EdgeRedisRepository(redis_client, key_prefix, logger)

        edge_redis_repository.remove_all_stored_elements()
    ...
    def add_edge_with_full_id_as_dict_test(self):
        ...
        edge_redis_repository = EdgeRedisRepository(redis_client, keys_prefix, logger)
        edge_redis_repository.get_all_edges = Mock(return_value=currently_stored_edges)

        current_timestamp = time.time()
        time_mock = Mock()
        time_mock.time = Mock(return_value=current_timestamp)
        with patch.object(edge_redis_repository_module, 'time', new=time_mock):
            edge_redis_repository.add_edge(new_edge_full_id, new_edge_status)
    ...
    def add_edge_with_update_existing_edge_test(self):
        ...
        edge_redis_repository = EdgeRedisRepository(redis_client, keys_prefix, logger)
        edge_redis_repository.exists_edge = Mock(return_value=True)
        edge_redis_repository.get_all_edges = Mock(return_value=currently_stored_edges)
        ...
        time_mock.time = Mock(return_value=current_timestamp)
        with patch.object(edge_redis_repository_module, 'time', new=time_mock):
            edge_redis_repository.add_edge(new_edge_full_id, new_edge_status, update_existing=True)
    ...
    def add_edge_with_no_update_existing_edge_and_edge_not_actually_existing_test(self):
        ...
        edge_redis_repository = EdgeRedisRepository(redis_client, keys_prefix, logger)
        edge_redis_repository.exists_edge = Mock(return_value=False)
        edge_redis_repository.get_all_edges = Mock(return_value=currently_stored_edges)
        ...
        time_mock.time = Mock(return_value=current_timestamp)
        with patch.object(edge_redis_repository_module, 'time', new=time_mock):
            edge_redis_repository.add_edge(new_edge_full_id, new_edge_status, update_existing=False)

        edge_redis_repository.exists_edge.assert_called_once_with(new_edge_full_id)
        redis_client.set.assert_called_once_with(
    ...
    def get_edge_with_full_id_as_dict_test(self):
        ...
        edge = edge_redis_repository.get_edge(edge_full_id)
    ...
    def get_edge_with_full_id_as_edge_identifier_test(self):
        ...
        edge = edge_redis_repository.get_edge(edge_identifier)
    ...
    def get_all_edges_test(self):
        ...
        edges = edge_redis_repository.get_all_edges()
    ...
    def exists_edge_test(self):
        ...
        edge_redis_repository.exists_edge(edge_full_id)
    ...
    def remove_edge_test(self):
        ...
        edge_redis_repository.remove_edge(edge_full_id)
