...
class TestEdgeRepository:
    ...
    def add_edge_with_full_id_as_dict_test(self):
        ...
        redis_client.set.assert_called_once_with(
            ...
            json.dumps({'edge_status': new_edge_status, 'addition_timestamp': current_timestamp}),
            ex=60,
        )
    ...
    def get_edge_with_full_id_as_dict_test(self):
        ...
        redis_client = Mock()
        redis_client.get = Mock(return_value=edge_value)
    ...
    def get_all_edges_test(self):
        ...
        edge_1_identifier = EdgeIdentifier(
            host=edge_1_host, enterprise_id=str(edge_1_enterprise_id), edge_id=str(edge_1_id))
        ...
        edge_2_identifier = EdgeIdentifier(
            host=edge_2_host, enterprise_id=str(edge_2_enterprise_id), edge_id=str(edge_2_id))

        current_redis_keys = [
            f'{edge_1_host}|{edge_1_enterprise_id}|{edge_1_id}',
            f'{edge_2_host}|{edge_2_enterprise_id}|{edge_2_id}',
        ]
        ...
        redis_client = Mock()
        redis_client.scan_iter = Mock(return_value=current_redis_keys)
        redis_client.mget = Mock(return_value=[edge_1_value, edge_2_value])
