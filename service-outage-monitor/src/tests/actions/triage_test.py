...
class TestTriage:
    ...
    async def poll_tickets_test(self):
        ...
        triage._filtered_ticket_details.assert_awaited_once_with(tickets)
    ...
    async def poll_tickets_with_bruin_tickets_returning_none_test(self):
        ...
        with patch.object(triage_module, 'uuid', side_effect=uuid_side_effect):
            await triage._poll_tickets()
    ...
    async def poll_tickets_with_filtered_tickets_and_dev_environment_test(self):
        ...
        triage._client_id_from_edge_status = Mock(return_value=85940)
        ...
        triage._filtered_ticket_details.assert_awaited_once_with(tickets)
        triage._compose_ticket_note_object.assert_called_once_with(edge_status, edge_event)
        triage._template_renderer._ticket_object_to_email_obj.assert_called_once_with(ticket_note_object)
    ...
    async def poll_tickets_with_filtered_tickets_and_production_environment_test(self):
        ...
        triage._filtered_ticket_details.assert_awaited_once_with(tickets)
        triage._compose_ticket_note_object.assert_called_once_with(edge_status, edge_event)
        triage._ticket_object_to_string.assert_called_once_with(ticket_note_object)
    ...
    async def poll_tickets_with_filtered_tickets_and_unknown_environment_test(self):
        ...
        triage._template_renderer._ticket_object_to_email_obj = Mock()
        triage._client_id_from_edge_status = Mock(return_value=85940)
        ...
        triage._filtered_ticket_details.assert_awaited_once_with(tickets)
        triage._compose_ticket_note_object.assert_called_once_with(edge_status, edge_event)
        triage._ticket_object_to_string.assert_not_called()
        triage._template_renderer._ticket_object_to_email_obj.assert_not_called()
    ...
    async def filtered_ticket_details_with_null_ticket_notes_test(self):
        ...
        triage._auto_resolve_tickets.assert_called_once_with(dict(
            ticketID=ticket_id,
            serial=edge_serial,
    ...
    async def filtered_ticket_details_with_existing_triage_and_timestamp_diff_lower_than_30_minutes_test(self):
        ...
        triage._extract_field_from_string.assert_called_once_with(
            ticket_notes, 'TimeStamp: '
        )
        triage._check_for_new_events.assert_not_awaited()
        triage._auto_resolve_tickets.assert_called_once_with(dict(
            ticketID=ticket_id,
            serial=edge_serial,
            notes=ticket_details[
    ...
    async def filtered_ticket_details_with_existing_triage_and_timestamp_diff_greater_than_30_minutes_test(self):
        ...
        triage._check_for_new_events.assert_awaited_once_with(
    ...
    async def check_for_new_events_with_meaningful_events_and_check_they_are_sorted_test(self):
        ...
        triage._compose_event_note_object.assert_any_call(events_data_sorted_by_timestamp)
    ...
    async def check_for_new_events_with_meaningful_events_and_event_note_less_than_event_limit_test(self):
        ...
        triage._compose_event_note_object.assert_has_calls([
            call(events_data)
        ], any_order=False)
    ...
    async def check_for_new_events_with_meaningful_events_and_production_environment_test(self):
        ...
        triage._compose_event_note_object.assert_called_once_with(events_data)
        event_bus.rpc_request.assert_has_awaits([
            call(
                'edge.status.request',
                {
    ...
    def compose_ticket_note_object(self):
        ...
        ticket_object = triage._compose_ticket_note_object(edges_to_report, events_to_report)
        ...
        assert triage._find_recent_occurence_of_event.called
