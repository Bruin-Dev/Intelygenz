...
class MonitoringMapRepository:
    ...
    async def map_bruin_client_ids_to_edges_serials_and_statuses(self):
        ...
        edge_list_response = await self._velocloud_repository.get_edges_for_triage()
    ...
    async def _process_edge_and_tickets(self, edge_full_id):
        ...
        async def process_edge_and_tickets():
            async with self._semaphore:
                ...
                self._logger.info(
                    ...
                    f"Took {(time.time() - start_time) // 60} minutes."
                )
        ...
        try:
            ...
            self._logger.error(
                f"[map-bruin-client-to-edges] An error occured while trying to process edge {edge_identifier} -> {e}"
            )
