...
class Route53Util:
    ...
    def _get_pod_name_nginx_ingress(self):
        ...
        ret = self._v1.list_namespaced_pod(namespace=self._nginx_ingress_namespace,
        ...
        if ret.items:
            ...
            print(f"No pod found for nginx ingress in namespace {self._nginx_ingress_namespace} "
    ...
    def _wait_pod_nginx_ingress_is_ready(self, pod_name_arg):
        nginx_ingress_pod_status = self._v1.list_namespaced_pod(namespace=self._nginx_ingress_namespace,
                                                                field_selector='metadata.name=' + pod_name_arg,
                                                                watch=False)
    ...
    def _do_action_in_route53_record(self, elb_hosted_zone_id,
                                     hosted_zone_to_change_id,
                                     elb_dns_ingress_hostname,
                                     update, delete):
        ...
        print(f"It's going to do action {action} in the the "
        ...
        response = self._route53_client.change_resource_record_sets(
           HostedZoneId=hosted_zone_to_change_id,
    ...
    def _wait_until_route53_change_is_propagated(self, change_id):
        ...
        change_result = self._route53_client.get_change(
            Id=change_id
        )
    ...
    def _do_necessary_actions(self, update, delete):
        ...
        change_id = self._do_action_in_route53_record(elb_canonical_hosted_zone_id,
                                                      hosted_zone_to_change_id,
                                                      elb_dns_ingress_hostname,
                                                      update, delete)
    ...
    def check_conditions(self):
        ...
        if not update_action and not delete_action:
            print("It's necessary provide an action to do: "
                  "-d/--delete True | -/--update True")
            exit(1)
        elif update_action and delete_action:
            print("It's only possible select True for one action: "
                  "-d/--delete True | -u/--update True")
            exit(1)
        elif update_action:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--record_alias_name',
                        type=str,
                        help='Record alias name',
                        required=True)
    parser.add_argument('-n', '--hosted_zone_name',
                        type=str,
