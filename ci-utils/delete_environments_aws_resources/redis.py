...
class RedisCluster:
    ...
    def _check_delete_redis_resources(redis_cluster_delete_result, redis_cache_subnet_group_result):
        ...
        redis_resources_delete_result.update({'redis_cluster': redis_resources_delete_result_redis,
    ...
    def _delete_cache_subnet_group(self, environment):
        logging.info("Checking if there is an ElastiCache Subnet Group for the environment {}".
                     format(environment))
    ...
    def _check_redis_cache_subnet_group(environment):
        ...
        redis_subnet_list_call = subprocess.Popen(['aws', 'elasticache', 'describe-cache-subnet-groups',
                                                   '--cache-subnet-group-name', redis_subnet_group, '--region',
                                                   'us-east-1'],
                                                  stdout=subprocess.PIPE, stderr=FNULL)
        try:
