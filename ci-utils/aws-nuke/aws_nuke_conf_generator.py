...
class AWSNukeConfigurationGenerator:
    ...
    def _generate_servicediscovery_namespace_rules(self, environment_name):
        ...
        if self._check_servicediscovery_namespace_exists(actual_namespaces, environment_name):
            ...
            logging.info("Exists namespace for service discovery of environment {} with id {}".format(environment_name,
                                                                                                      namespace_id))
            aws_nuke_entry_namespace.update({'ServiceDiscoveryNamespace': self._create_dict_entry_aws_nuke_value_invert(
                namespace_id)})
        else:
        ...
        if len(actual_ecs_clusters) > 0:
            for element in actual_ecs_clusters:
                if environment_name in element:
                    ...
                    aws_nuke_entry_cluster.append(self._generate_rule_aws_nuke_equal_to_value(
                        'ECSCluster', element))
                    break
