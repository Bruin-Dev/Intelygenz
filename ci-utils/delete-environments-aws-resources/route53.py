...
class Route53:
    ...
    def delete_environment_record_set(self, environment):
        ...
        if hosted_zone_information["exists"]:
            logging.info(
                "Hosted zone {} exists and its identifier is {}".format(
                    self._hosted_zone_for_records_name, hosted_zone_information["hosted_zone_id"]
                )
            )
            ...
            if environment_record["exists"]:
                logging.info(
                    "There is a record set for environment {} in hosted zone with identifier {}".format(
                ...
                subprocess.call(
                    [
                        ...
                        "us-east-1",
                        "--change-batch",
                        data_record_to_delete_json,
                    ],
                    stdout=FNULL,
                )
                logging.info(
                    "Record set for environment {} in hosted zone {} was successfully deleted".format(
                        environment, hosted_zone_id
                    )
                )
            else:
                logging.error(
                    "There isn't a record set for environment {} in hosted zone with identifier {}".format(
                        environment, hosted_zone_information["hosted_zone_id"]
                    )
                )
        else:
    ...
    def _check_hosted_zone_for_records(self):
        ...
        hosted_zones_list_call = subprocess.Popen(
            ["aws", "route53", "list-hosted-zones", "--region", "us-east-1"], stdout=subprocess.PIPE, stderr=FNULL
        )
    ...
    def _check_resource_record_environment(self, hosted_zone_id, environment):
        logging.info(
            f"Checking if there is a record set for environment {environment} in hosted "
            f"zone with id {hosted_zone_id}"
        )
        resource_record_environment = {"exists": False}
        resources_records_set_list_call = subprocess.Popen(
            [
                "aws",
                "route53",
    ...
    def _get_json_for_delete_record(record_information):
        ...
        data_record_to_delete_changes = [{"Action": "DELETE", "ResourceRecordSet": record_information}]
