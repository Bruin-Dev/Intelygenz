...
class SecurityGroups:
    ...
    def _get_security_groups(cluster):
        security_groups = []
        security_groups_list_call = subprocess.Popen(
            [
                "aws",
                "ec2",
                "describe-security-groups",
                "--filters",
                "Name=tag:Environment, Values=" + cluster,
                "--region",
                "us-east-1",
            ],
    ...
    def delete_security_groups(self, environment):
        ...
        if security_groups_cluster_size > 0:
            ...
            for element in security_groups_cluster:
                ...
                logging.info(
                    "Deleting Security Group with name {} and id {}".format(security_group_name, security_group_id)
                )
                ...
                common_utils_instance.check_current_state_call(
                    remove_security_group, "Security Group", security_group_name
                )
                if remove_security_group != 0:
                    ...
                    while common_utils_instance.can_retry_call(security_group_delete_try) and current_exit_code != 0:
                        logging.info("security_group_delete_try is {}".format(security_group_delete_try))
                    ...
                    common_utils_instance.check_current_state_call(
                        current_exit_code, "Security Group", security_group_name
                    )
        else:
