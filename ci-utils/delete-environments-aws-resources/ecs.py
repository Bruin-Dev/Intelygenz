...
class EcsServices:
    ...
    def _delete_ecs_service(environment, service_name, service_id):
        logging.info(
            "Update desired count of service {} with id {} of ECS cluster {} to 0 ".format(
                service_name, service_id, environment
            )
        )
        subprocess.call(
            ["aws", "ecs", "update-service", "--cluster", environment, "--service", service_id, "--desired-count", "0"],
            stdout=FNULL,
        )
        logging.info("Deleting service {} with id {} of cluster {}".format(service_name, service_id, environment))
        subprocess.call(
            ["aws", "ecs", "delete-service", "--cluster", environment, "--service", service_id], stdout=FNULL
        )
        ...
        try:
            ...
            if actual_tasks is not None:
                ...
                for element in result:
                    ...
                    subprocess.call(
                        ["aws", "ecs", "stop-task", "--cluster", environment, "--task", task_identifier], stdout=FNULL
                    )
    ...
    def _list_and_delete_all_services(self, environment):
        actual_services_call = subprocess.Popen(
            ["aws", "ecs", "list-services", "--cluster", environment, "--region", "us-east-1"],
                ...
                for key in services[index]:
                    logging.info(
                        "Deleting service with name {} and id {} from cluster {}".format(
                            services[index]["service_name"], services[index]["service_id"], environment
                        )
                    )
                    remove_service_cmd = (
                    ...
                    common_utils_instance.check_current_state_call(
                        remove_service_result, "Service from Service Discovery", services[index]["service_id"]
                    )
                    if remove_service_result != 0:
                        service_from_discovery_service_delete_try = 0
                        current_exit_code = 0
                        while (
                            common_utils_instance.can_retry_call(service_from_discovery_service_delete_try)
                            and current_exit_code != 0
                        ):
                            (
                                current_exit_code,
                                service_from_discovery_service_delete_try,
                            ) = common_utils_instance.retry_call(
                                remove_service_cmd, service_from_discovery_service_delete_try
                            )
                        common_utils_instance.check_current_state_call(
                            current_exit_code, "Service from Service Discovery", services[index]["service_id"]
                        )
    ...
    def delete_servicediscovery(self, environment):
        logging.info(
            "Checking if there are AWS resources related to the service discovery of the {}"
        ...
        if cluster_information["exists"] and not cluster_information["inactive"]:
            logging.info(
                "There is an ECS cluster with the name {} with {} services active and {} tasks running".format(
                    environment,
                    cluster_information["ecs_cluster_information"]["activeServicesCount"],
    ...
    def delete_ecs_cluster(self, environment):
        ...
        if cluster_information["exists"] and not cluster_information["inactive"]:
            logging.info(
                "There is an ECS cluster for the environment {} with {} services active and {} tasks running".format(
                    environment,
                    cluster_information["ecs_cluster_information"]["activeServicesCount"],
                    cluster_information["ecs_cluster_information"]["runningTasksCount"],
                )
            ...
            cmd_call_remove_ecs_cluster = (
                "aws, ecs, delete-cluster, --cluster, " + environment + ", --region" + ", us-east-1"
            )
            remove_ecs_cluster_call = subprocess.call(
                cmd_call_remove_ecs_cluster.split(", "), stdout=FNULL, stderr=FNULL
            )
            common_utils_instance = common_utils_module.CommonUtils()
            common_utils_instance.check_current_state_call(remove_ecs_cluster_call, "ECS Cluster", environment)
            if remove_ecs_cluster_call != 0:
                ecs_cluster_delete_try = 0
                current_exit_code = remove_ecs_cluster_call
                while common_utils_instance.can_retry_call(ecs_cluster_delete_try) and current_exit_code != 0:
                ...
                common_utils_instance.check_current_state_call(current_exit_code, "Security Group", environment)
        elif cluster_information["exists"] and cluster_information["inactive"]:
            logging.info(
                "There is an ECS cluster for the environment {} but it's not going to be deleted because "
    ...
    def _check_if_ecs_cluster_exists(self, environment):
        ...
        cluster_exists_check_call = subprocess.Popen(
            ["aws", "ecs", "describe-clusters", "--cluster", environment], stdout=subprocess.PIPE, stderr=FNULL
        )
        ...
        if len(cluster_exists_check_result) > 0:
            if self._check_ecs_cluster_is_active(cluster_exists_check_result[0]):
                cluster_exists.update(
                    {"exists": True, "inactive": False, "ecs_cluster_information": cluster_exists_check_result[0]}
                )
            else:
                cluster_exists.update({"exists": True, "inactive": True})
    ...
    def _check_ecs_cluster_is_active(cluster_info):
        ...
        if cluster_info["status"] == "ACTIVE" or (
            ...
            and (
                cluster_info["registeredContainerInstancesCount"] > 0
                or cluster_info["runningTasksCount"] > 0
                or cluster_info["pendingTasksCount"] > 0
                or cluster_info["activeServicesCount"] > 0
            )
        ):
