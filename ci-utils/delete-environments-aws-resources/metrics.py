...
class Metrics:
    ...
    def _get_cluster_dashboard(environment):
        ...
        cluster_dashboard_list_call = subprocess.Popen(
            ["aws", "cloudwatch", "list-dashboards"], stdout=subprocess.PIPE, stderr=FNULL
        )
    ...
    def _delete_dashboard(self, environment):
        ...
        if cluster_dashboard["exists"]:
            ...
            subprocess.call(
                ["aws", "cloudwatch", "delete-dashboards", "--dashboard-names", dashboard_name], stdout=FNULL
            )
        else:
    ...
    def _get_cluster_alarms(environment):
        ...
        cluster_alarms_list_call = subprocess.Popen(
            ["aws", "cloudwatch", "describe-alarms", "--region", "us-east-1"], stdout=subprocess.PIPE, stderr=FNULL
        )
    ...
    def _get_log_metric_for_cluster(environment):
        ...
        metrics_logs_list_call = subprocess.Popen(
            ["aws", "logs", "describe-metric-filters", "--log-group-name", environment, "--region", "us-east-1"],
        ...
        try:
            ...
            if metrics_logs_list is not None and len(metrics_logs_list) > 0:
                ...
                metrics_logs_for_cluster.update(
                    {"has_log_metrics": True, "log_metrics_filters": metrics_logs_for_cluster_elements}
                )
    ...
    def _delete_log_group(self, environment):
        if self._check_log_group_exists(environment):
            ...
            if log_metrics_for_cluster["has_log_metrics"]:
                logging.info(
                    "The environment {} has associated {} log metric/s filter/s".format(
                        environment, len(log_metrics_for_cluster["log_metrics_filters"])
                    )
                )
                self._delete_log_metrics(environment, log_metrics_for_cluster["log_metrics_filters"])
            else:
                logging.error(
                    "The environment {} doesn't have any log metric associated to its log group".format(environment)
                )
