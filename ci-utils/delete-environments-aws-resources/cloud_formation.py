...
class CloudFormation:
    ...
    def _check_delete_result(self, cloud_formation_environment_exists, environment):
        ...
        if cloud_formation_environment_exists:
            ...
            logging.info(
                "There is a CloudFormation resource for environment {} with name {}".format(environment, stack_name)
            )
            logging.info("CloudFormation resource for environment {} it's going to be deleted".format(environment))
            cmd_call_remove_cloudformation_stack = (
            ...
            common_utils_instance.check_current_state_call(
                remove_cloudformation_stack, "CloudFormation stack", stack_name
            )
            if remove_cloudformation_stack == 0:
                cloud_formation_delete_result.update(
                    {"exists": cloud_formation_environment_exists, "success_delete": True}
                )
            else:
                cloud_formation_delete_result.update(
                    {"exists": cloud_formation_environment_exists, "success_delete": False}
                )
        else:
    ...
    def _check_cloud_formation_for_environment(self, environment):
        ...
        cloud_formation_list_call = subprocess.Popen(
            [
                "aws",
                "cloudformation",
                "list-stack-resources",
                "--stack-name",
                cloud_formation_name,
                "--region",
                "us-east-1",
            ],
        ...
        try:
            ...
            if len(cloud_formation_list) > 0:
                cloud_formation.update({"exists": True, "cloud_formation_information": cloud_formation_list[0]})
