...
class S3Buckets:
    ...
    def _delete_s3bucket_files_terraform_infrastructure(self, environment):
        logging.info(
            f"Checking if there are directories in s3 bucket {self._s3_bucket_backend_terraform} "
        ...
        if s3buckets_check["s3_buckets"]:
            logging.info(
                f"The environment {environment} has associated {len(s3buckets_check['buckets_list'])} "
            ...
            logging.error(
                "The environment {} hasn't any associated directories in bucket {} to store Terraform "
    ...
    def _check_file_exists_in_s3_bucket(bucket_name, file):
        ...
        s3_bucket_search_call = subprocess.Popen(
            ["aws", "s3", "ls", s3_bucket_to_search], stdout=subprocess.PIPE, stderr=FNULL
        )
    ...
    def _delete_content_of_bucket_recursively(bucket_name):
        delete_bucket_result = subprocess.call(
            ["aws", "s3", "rm", bucket_name, "--recursive", "--region", "us-east-1"], stdout=FNULL
        )
