...
class IamRolesUtil:
    ...
    def _create_botocore_service_session(self, service_name):
        ...
        if aws_profile:
            ...
            session = boto3.session.Session(
                aws_access_key_id=aws_acces_key_id, aws_secret_access_key=aws_secret_access_key
            )
        else:
    ...
    def _create_necessary_tags_map(project_tag, project_role_tag, project_env_tag):
        if project_tag and project_role_tag:
            necessary_tags = [
                ...
                {"Key": "Project-Role", "Value": project_role_tag},
                {"Key": "Project-Env", "Value": project_env_tag},
            ]
    ...
    def _get_iam_roles_of_eks_with_specific_profile(self, iam_botocore_session, project, project_role, project_env):
        ...
        while has_more_data:
            if marker:
                resp = iam_botocore_session.list_roles(Marker=marker)
            else:
            ...
            for role in roles:
                ...
                role_arn = role["Arn"]
                role_tags = iam_botocore_session.list_role_tags(RoleName=rolename)
                ...
                if role_tags_l:
                    ...
                    if all_necessary_tags_in_role_tags:
                        ...
                        roles_filtered.append({"role_name": rolename, "role_arn": role_arn})
    ...
    def _update_kubernetes_aws_auth_cm(self, iam_roles_filtered):
        ...
        aws_auth_cm = self._kubernetes_core_v1_api.list_namespaced_config_map(
            namespace="kube-system", field_selector="metadata.name=aws-auth", watch=False, pretty="true"
        )
        if aws_auth_cm.items:
            ...
            aws_auth_cm_map_roles_u = self._update_map_roles_items_aws_auth_cm(
                iam_roles_filtered, aws_auth_cm_map_roles
            )
            ...
            if aws_auth_cm_map_roles == aws_auth_cm_map_roles_u:
                ...
                self._kubernetes_core_v1_api.replace_namespaced_config_map(
                    namespace="kube-system", name="aws-auth", body=aws_auth_cm.items[0]
                )
        else:
    ...
    def _check_specific_cluster_role_exists(self, role_name):
        ...
        cluster_role = self._kubernetes_rbac_v1_api.list_cluster_role(
            field_selector="metadata.name=" + role_name, pretty="true"
        )
    ...
    def _create_specific_cluster_role(self, role_name):
        ...
        if role_permissions_are_defined:
            ...
            print(
                f"Permissions for role {role_name} are predefined and are the following "
    ...
    def _check_specific_cluster_role_binding_exists(self, cluster_role_binding_name):
        cluster_role_binding = self._kubernetes_rbac_v1_api.list_cluster_role_binding(
            field_selector="metadata.name=" + cluster_role_binding_name, watch=False
        )
        cluster_role_binding_info = {"exists": False}
        if cluster_role_binding.items:
            print(f"ClusterRoleBinding {cluster_role_binding_name} already exists")
            cluster_role_binding_info.update({"exists": True, "info": cluster_role_binding.items[0]})
        else:
    ...
    def _generate_cluster_role_binding_body(self, role_name, cluster_role_binding_name, iam_roles_filtered):
        ...
        role_subjects = [
            {"api_group": f"{role_ref_api_group}", "kind": "User", "name": r["role_name"], "namespace": None}
    ...
    def _create_specific_cluster_role_binding(self, body_cluster_role_binding):
        ...
        print(
            f"ClusterRoleBinding with name {cluster_role_binding_name} is going to be created "
            f"with the following content"
        )
        print(f"{json.dumps(body_cluster_role_binding, indent=4, default=str)}")
        self._kubernetes_rbac_v1_api.create_cluster_role_binding(body=body_cluster_role_binding)
    ...
    def _update_specific_cluster_role_binding(self, body_cluster_role_binding, cluster_role_binding_info):
        ...
        cluster_role_binding_actual_subjects_l = self._convert_cluster_role_binding_subjects_without_v1_subjects_type(
            cluster_role_binding_actual_subjects
        )
        ...
        if body_cluster_role_binding_subjects == cluster_role_binding_actual_subjects_l:
            ...
            self._kubernetes_rbac_v1_api.replace_cluster_role_binding(
                name=cluster_role_binding_name, body=body_cluster_role_binding
            )
    ...
    def do_necessary_actions(self):
        ...
        iam_roles_filtered = self._get_iam_roles_of_eks_with_specific_profile(
            iam_botocore_session, project, project_role, project_env
        )
        if iam_roles_filtered:
            ...
            cluster_role_binding_body_k8s = self._generate_cluster_role_binding_body(
                project_role, cluster_role_binding_name, iam_roles_filtered
            )
            if not cluster_role_binding_info["exists"]:
                self._create_specific_cluster_role_binding(cluster_role_binding_body_k8s)
            else:
                self._update_specific_cluster_role_binding(cluster_role_binding_body_k8s, cluster_role_binding_info)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--project-tag", type=str, help="Project tag to get users", required=True)
    parser.add_argument("--project-role-tag", type=str, help="Project-Role tag to get users", required=True)
    parser.add_argument("--project-env-tag", type=str, help="Project-Env tag to get users", required=True)
    parser.add_argument("-a", "--aws-profile", type=str, help="AWS profile to use", required=False)
    parser.add_argument(
        "-r", "--aws-region", type=str, help="Region to use in AWS", default="us-east-1", required=False
    )
