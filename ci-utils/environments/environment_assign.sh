...
if [[ "${CI_COMMIT_REF_SLUG}" != "master" ]]; then
  ...
  export PROMETHEUS_INGRESS_HOSTNAME="${TF_VAR_CURRENT_ENVIRONMENT}-prometheus.mettel-automation.net"
  export PROMETHEUS_INGRESS_EXTERNAL_URL="https://${PROMETHEUS_INGRESS_HOSTNAME}"
  export GRAFANA_INGRESS_HOSTNAME="${TF_VAR_CURRENT_ENVIRONMENT}-grafana.mettel-automation.net"
  ...
  export TAG=$(git tag | sort --version-sort | grep "${MODULE_NAME}" | tail -n -1 | sed -e "s/^${MODULE_NAME}-v//")
