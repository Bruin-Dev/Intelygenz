...
class EcrUtil:
    ...
    def _check_is_project_repository(self, repository_name, repository_arn):
        ...
        repository_tags_resp = self._client.list_tags_for_resource(
    ...
    def _obtain_latest_image_of_repository_from_environment(self, environment, repository):
        ...
        if environment != self._environment_id_production:
            images_of_environment = self._get_latest_image_of_repository_for_ephemeral_environment(environment, repository)
            if images_of_environment:
    ...
    def _obtain_latest_image_of_repositories_from_environment(self, environment, repositories):
        ...
        for repository in repositories:
            latest_docker_image_for_repository = self._obtain_latest_image_of_repository_from_environment(environment,
                                                                                                          repository)
            ...
            for image in docker_images['imageIds']:
                ...
                if image_tag:
                    if all_images or ("latest" not in image_tag and not all_images):
                        image_tag_split = image_tag.split("-")
                        if (environment == self._environment_id_production and len(image_tag_split) == 1)\
                                or (len(image_tag_split) == 2 and image_tag_split[0] == environment):
                            images_of_environment.append(
                                {
                                    'imageTag': image_tag,
                                    'imagePushedAt': self._obtain_image_pushed_time(repository, image_tag)
    ...
    def _obtain_images_to_delete_of_repositories_from_environment(self, environment, ecr_repositories, only_old):
        ...
        for repository in ecr_repositories:
            if only_old:
                images_of_environment = self._obtain_images_of_repository_from_environment_in_repository(environment,
                                                                                                         repository,
                                                                                                         False)
            else:
                images_of_environment = self._obtain_images_of_repository_from_environment_in_repository(environment,
            ...
            if images_of_environment:
                if only_old and len(images_of_environment) > 2:
                    ...
                    logging.error(f"Repository {repository} has {len(images_of_environment)} oldest images "
                    ...
                    docker_images_in_repositories.append(
                        {
                            'repository': repository,
                            'images_to_delete': [i['imageTag'] for i in images_of_environment]
                        }
                    )
            else:
                logging.error(f"Repository {repository} hasn't images "
                              f"for environment {environment}. No image is going to be deleted")
        ...
        if environment:
            ...
            if parser.parse_args().oldest_images:
                repositories_to_delete_images, docker_images_to_delete_in_repositories = self.\
                    _obtain_images_to_delete_of_repositories_from_environment(environment,
                                                                              ecr_repositories,
                                                                              True)
            else:
                repositories_to_delete_images, docker_images_to_delete_in_repositories = \
                        self._obtain_images_to_delete_of_repositories_from_environment(environment,
                                                                                       ecr_repositories,
                                                                                       False)
            ...
            if print_results:
                if get_all:
                    for images_for_ecr_repository in latest_images_for_ecr_repositories:
                        ...
                        logging.info(f'The result of obtaining the latest docker images for the repository '
        ...
        if parser.parse_args().delete or parser.parse_args().get:
            ...
            if parser.parse_args().delete:
                ...
                if all_repositories:
                    ...
                    logging.info(f"It's going to obtain images for repository {repository} in environment {environment}")
                    latest_images_from_ecr_repositories = self._obtain_latest_image_of_repository_from_environment(
                        environment, repository)
                self._print_results(latest_images_from_ecr_repositories, parser.parse_args().print_results,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-e', '--environment',
                        type=str,
                        help='Name of the environment to get images',
                        required=True)
    parser.add_argument('-p', dest='print_results', action='store_true',
                        help='Flag to indicate whether the results are to be printed on the screen',
                        required=False,
                        default=False)
    parser.add_argument('-a', dest='all_repositories', action='store_true',
                        help='Indicate if all ECR repositories are going to be explored to delete older images',
                        required=False,
                        default=False)
    parser.add_argument('-o', dest='oldest_images', action='store_true',
