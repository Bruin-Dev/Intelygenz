...
class RedisCluster:
    @staticmethod
    def _check_redis_cluster_exists(cluster):
    ...
    def _check_redis_cluster_is_deleted(self, cluster, start_time):
        timeout = start_time + 60 * 5
        correct_exit = False
        actual_time = time.time()
        while timeout > actual_time:
            redis_cluster_status = self._check_redis_cluster_exists(cluster)
    ...
    def delete_redis_cluster(self, cluster):
        ...
        if redis_cluster['exists']:
            ...
            self._check_redis_cluster_is_deleted(cluster, start_time)
        else:
